...
from savnce_baselines.common.utils import poll_checkpoint_folder
...
class BaseRLTrainer(BaseTrainer):
    ...
    def eval(self, eval_interval=1, prev_ckpt_ind=-1, use_last_ckpt=False) -> None:
        ...
        self.has_printed_eval_config = False
    ...
    def get_model_parameters(self, model, target_path = ''):
        module = model
        if target_path:
        ...
        for name, child in module.named_children():
            ...
            learnable_params = sum(p.numel() for p in child.parameters() if p.requires_grad)
            if total_params > 0:
                modules_params[name] = (total_params, learnable_params)
        return model_total_params, model_learnable_params, modules_params
