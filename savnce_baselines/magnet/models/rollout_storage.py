...
class ExternalMemory:
    def __init__(self, num_envs, total_size, capacity, dim, num_copies=1):
        ...
        self.dim = dim
        self.masks = torch.zeros(num_envs, self.total_size) # (batch_size, total_size) (10, 300)
        self.memory = torch.zeros(self.total_size, num_copies, num_envs, self.dim) # (total_size, num_copies, batch_size, dim) (300, 151, 10, 832)
        self.idx = 0

    def insert(self, em_features, not_done_masks):
        # Update memory storage and add new memory as a valid entry
        # em_features (batch_size, dim), not_done_masks (batch_size, ) 0 for done, 1 for not done
        self.memory[self.idx].copy_(em_features.unsqueeze(0))
        # Account for overflow capacity
        capacity_overflow_flag = self.masks.sum(1) == self.capacity # (batch_size, )
