...
from savnce_baselines.common.utils import observations_to_image
from savnce_baselines.common.base_trainer import BaseRLTrainer
from savnce_baselines.common.baseline_registry import baseline_registry
from savnce_baselines.common.env_utils import construct_envs
from savnce_baselines.common.environments import get_env_class
from savnce_baselines.common.tensorboard_utils import TensorboardWriter
from savnce_baselines.common.utils import (
    batch_obs,
    generate_video,
    linear_decay,
    plot_top_down_map,
...
from savnce_baselines.common.ddp_utils import (
    EXIT,
    REQUEUE,
...
from savnce_baselines.magnet.models.goal_descriptor import GoalDescriptor
from savnce_baselines.magnet.ppo.policy import SAVNCE_Policy
from savnce_baselines.magnet.ppo.ppo import PPO
from savnce_baselines.magnet.models.rollout_storage import RolloutStorage, ExternalMemory
...
@baseline_registry.register_trainer(name="savnce_ppo")
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        goal_descriptor_cfg = ppo_cfg.GOAL_DESCRIPTOR
        self.actor_critic = SAVNCE_Policy(
            observation_space=self.envs.observation_spaces[0],
            action_space=self.action_space,
            smt_cfg=smt_cfg,
            ppo_cfg=ppo_cfg,
            goal_descriptor_cfg=goal_descriptor_cfg,
            goal_sensor_uuids=self.config.TASK_CONFIG.TASK.GOAL_SENSOR_UUID,
            use_category_input=has_distractor_sound
        )

        if ppo_cfg.use_goal_descriptor:
            smt = self.actor_critic.net.smt_state_encoder
            self.goal_descriptor = GoalDescriptor(
                    observation_space=observation_space,
                    action_space=self.action_space,
                    batch_size=self.envs.num_envs,
                    config=goal_descriptor_cfg,
                    device=self.device
                )
            if not goal_descriptor_cfg.online_training:
                self.goal_descriptor.freeze_encoders()
    ...
    def save_checkpoint(
        ...
        if self.config.RL.PPO.use_goal_descriptor:
            checkpoint["goal_descriptor"] = self.goal_descriptor.state_dict()
    ...
    def try_to_resume_checkpoint(self):
        ...
        if len(checkpoints) == 0:
            ...
            if self.config.RL.PPO.use_goal_descriptor:
                self.goal_descriptor.load_state_dict(ckpt_dict["goal_descriptor"])
            ...
            logger.info(f"Resuming checkpoint {last_ckpt} at {count_steps} frames")
    ...
    def _collect_rollout_step(
        ...
        # # update goal descriptor sensor before policy network using it
        # if self.config.RL.PPO.use_goal_descriptor:
        #     gd_step_observation = {k: v[rollouts.step] for k, v in rollouts.observations.items()}
        #     gd_dones = (1 - rollouts.masks[rollouts.step].squeeze()) > 0
        #     gd_prev_actions = rollouts.prev_actions[rollouts.step]
        #     step_embedding, step_predict = self.goal_descriptor.update(gd_step_observation, gd_dones, gd_prev_actions)
        #     gd_observation = torch.cat([step_embedding, step_predict], dim=-1)
        #     rollouts.observations['goal_descriptor'][rollouts.step].copy_(gd_observation)
        # sample actions
        with torch.no_grad():
            step_observation = {
        ...
        # for i in range(len(infos)):
        #     if dones[i] and infos[i]['na'] > 10:
        #         print(f"{self.device}, {infos[i]['na']}")
        ...
        # update goal descriptor sensor before inserting into rollouts
        if self.config.RL.PPO.use_goal_descriptor:
            self.goal_descriptor.update(batch, dones, actions)
    ...
    def train_goal_descriptor(self, rollouts, output_format='ACCDDOA'):
        self.goal_descriptor.train()
        total_loss = 0.0
        self.goal_descriptor.optimizer.zero_grad()
        for i, episode in enumerate(self.goal_descriptor.data_generator.process(rollouts)):
            prev_actions = episode['prev_action']
            if output_format == 'ACCDDOA':
                gt = episode['oracle_accddoa']
            else:
                raise ValueError(f"Invalid output format: {output_format}")
            predict = self.goal_descriptor(episode, prev_actions)
            loss = self.goal_descriptor.loss_fn(predict, gt)
            total_loss = total_loss + loss
            self.goal_descriptor.metrics.update_metric(predict.detach().cpu(), gt.detach().cpu())
        total_loss = total_loss / (i + 1)
        self.goal_descriptor.before_backward(total_loss)
        total_loss.backward()
        self.goal_descriptor.optimizer.step()
        metrics = self.goal_descriptor.metrics.compute_metric()
        return total_loss.item(), i + 1, metrics
    ...
    #     # if self.config.RL.PPO.use_goal_descriptor:
    #     #     self.goal_descriptor.update(batch, None)
    ...
    def _eval_checkpoint(
        ...
        if self.config.DISPLAY_RESOLUTION != model_resolution:
            ...
            for sensor in ['depth', 'rgb']:
                sensor_obs_space = observation_space.spaces[sensor]
                observation_space.spaces[sensor] = spaces.Box(
                    low=sensor_obs_space.low.min(),
                    high=sensor_obs_space.high.max(),
                    shape=(model_resolution, model_resolution, sensor_obs_space.shape[-1]),
                    dtype=sensor_obs_space.dtype
                )
        else:
            observation_space = self.envs.observation_spaces[0]
        self._setup_actor_critic_agent(ppo_cfg, observation_space)

        if not self.has_printed_eval_config:
            logger.info(f"{self.config.EVAL.SPLIT} config: {config}")
            self.has_printed_eval_config = True
        logger.info(f"=======current_ckpt: {checkpoint_path}=======")
        self.agent.load_state_dict(ckpt_dict["state_dict"])
        self.actor_critic = self.agent.actor_critic
        self.actor_critic.eval()
        if self.config.RL.PPO.use_goal_descriptor and "goal_descriptor" in ckpt_dict:
            self.goal_descriptor.load_state_dict(ckpt_dict["goal_descriptor"])
            self.goal_descriptor.eval()
        self.metric_uuids = []
        ...
        batch = batch_obs(
            obs_copy,
            self.device,
            skip_list=['view_point_goals', 'intermediate', 'oracle_action_sensor']
        )
        ...
        if self.config.RL.PPO.use_goal_descriptor:
            self.goal_descriptor.update(
                batch, 
                [torch.tensor([False], dtype=torch.bool, device=self.device)] * self.envs.num_envs, 
                torch.zeros((self.envs.num_envs, 1), dtype=torch.long, device=self.device))
            ...
            for i in range(len(descriptor_pred_gt)):
                ...
                if ppo_cfg.GOAL_DESCRIPTOR.output_format == 'ACCDDOA':
                    ...
                    raise ValueError(f"Invalid output format: {ppo_cfg.GOAL_DESCRIPTOR.output_format}")
                geodesic_distance = -1
                pair = (descriptor_pred, descriptor_gt, geodesic_distance)
        ...
        while (
            ...
            for i in range(self.envs.num_envs):
                ...
                if not_done_masks[i].item() == 0:
                    ...
                    episode_stats['audio_duration'] = int(current_episodes[i].goals[0].duration)
                    episode_stats['gt_na'] = int(current_episodes[i].info['num_action'])
                    episode_stats['onset_step'] = int(current_episodes[i].goals[0].onset / self.config.TASK_CONFIG.SIMULATOR.STEP_TIME)
                    episode_stats['offset_step'] = int(current_episodes[i].goals[0].offset / self.config.TASK_CONFIG.SIMULATOR.STEP_TIME)
                    if self.config.RL.PPO.use_goal_descriptor:
                        descriptor_pred_batch = torch.from_numpy(np.stack([pair[0] for pair in descriptor_pred_gt[i]], axis=0))
                        descriptor_gt_batch = torch.from_numpy(np.stack([pair[1] for pair in descriptor_pred_gt[i]], axis=0))
                        self.goal_descriptor.metrics.update_metric(descriptor_pred_batch, descriptor_gt_batch)
                    ...
                    if len(self.config.VIDEO_OPTION) > 0:
                        ...
                        if 'sound' in current_episodes[i].info:
                            ...
                            sound = current_episodes[i].goals[0].sound_id.split('/')[1][:-4]
                        generate_video(
                    ...
                    if "top_down_map" in self.config.VISUALIZATION_OPTION:
                        top_down_map = plot_top_down_map(infos[i],
                                                         dataset=self.config.TASK_CONFIG.SIMULATOR.SCENE_DATASET)
                        scene = current_episodes[i].scene_id.split('/')[3]
                        top_down_map_dir = os.path.join(self.config.MODEL_DIR, 'top_down_map')
                        os.makedirs(top_down_map_dir, exist_ok=True)
                        filename = f"{config.EVAL.SPLIT}_{scene}_{current_episodes[i].episode_id}_{infos[i]['spl']:.3f}.png"
                        if top_down_map.dtype != np.uint8:
                            top_down_map = np.clip(top_down_map, 0, 255).astype(np.uint8)
                        Image.fromarray(top_down_map).save(os.path.join(top_down_map_dir, filename), format="PNG", compress_level=0)
            if not self.config.RL.PPO.use_goal_descriptor:
            ...
            if self.config.RL.PPO.use_goal_descriptor and len(envs_to_pause) > 0:
                self.goal_descriptor.episodic_memory.pop_at(envs_to_pause)
        
        # save descriptor_pred_gt to a separate file
        if self.config.RL.PPO.use_goal_descriptor and 'test' in config.EVAL.SPLIT:
            descriptor_pred_gt_file = os.path.join(config.TENSORBOARD_DIR,
                                                        '{}_descriptor_pred_gt_{}.json'.format(config.EVAL.SPLIT, config.SEED))
            descriptor_pred_gt_dict = {}
            for key, value in stats_episodes.items():
                if 'descriptor_pred_gt' in value:
                    descriptor_pred_gt_dict[key] = value['descriptor_pred_gt']
                    del value['descriptor_pred_gt']
            with open(descriptor_pred_gt_file, 'w') as fo:
                json.dump({','.join(key): value for key, value in descriptor_pred_gt_dict.items()}, fo, cls=NpEncoder)
        ...
        logger.info(f"Average episode reward: {episode_reward_mean:.6f} in {config.EVAL.SPLIT} split with {num_episodes} episodes")
        for metric_uuid in self.metric_uuids:
            logger.info(f"Average episode {metric_uuid}: {episode_metrics_mean[metric_uuid]:.6f}")
        if self.config.RL.PPO.use_goal_descriptor:
            gd_metrics = self.goal_descriptor.metrics.compute_metric()
            metrics_str = ', '.join(f"{metric}: {value:.4f}" for metric, value in gd_metrics.items())
            logger.info(f"goal_descriptor, {metrics_str}")
        ...
        if not config.EVAL.SPLIT.startswith('test'):
            ...
            if self.config.RL.PPO.use_goal_descriptor:
                for metric, value in gd_metrics.items():
                    writer.add_scalar(f"{config.EVAL.SPLIT}_gd/{metric}", value, checkpoint_index)
        self.envs.close()
