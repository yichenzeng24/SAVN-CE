...
from savnce_baselines.savi.models.rollout_storage import RolloutStorage
from savnce_baselines.common.tensorboard_utils import TensorboardWriter
from savnce_baselines.common.utils import batch_obs, format_number, linear_decay
...
from savnce_baselines.savi.ddppo.algo.ddppo import DDPPO
from savnce_baselines.savi.models.belief_predictor import BeliefPredictor, BeliefPredictorDDP
from savnce_baselines.savi.ppo.ppo_trainer import PPOTrainer
from savnce_baselines.savi.ppo.policy import AudioNavSMTPolicy, AudioNavBaselinePolicy


@baseline_registry.register_trainer(name="savi_ddppo")
class DDPPOTrainer(PPOTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        if self.config.RL.DDPPO.pretrained:
            ...
            if ppo_cfg.use_belief_predictor and 'belief_predictor' in pretrained_state:
                self.belief_predictor.load_state_dict(pretrained_state["belief_predictor"])
                logger.info(f"Loaded pretrained weights for belief predictor from {self.config.RL.DDPPO.pretrained_weights}")
    ...
    def train(self) -> None:
        ...
        if self.world_rank == 0:
            ...
            if ppo_cfg.use_belief_predictor:
                logger.info(f"belief predictor: \n{self.belief_predictor}")
                total_params, learnable_params, modules_params = self.get_model_parameters(self.belief_predictor)
                logger.info(f"belief descriptor: {format_number(total_params)}, learnable parameters: {format_number(learnable_params)}")
                for name, (total_params, learnable_params) in modules_params.items():
                    logger.info(f"{name}: {format_number(total_params)}, learnable parameters: {format_number(learnable_params)}")
