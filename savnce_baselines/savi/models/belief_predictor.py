...
from savnce.tasks.nav import SpectrogramSensor, LocationBelief, CategoryBelief, Category
from savnce_baselines.savi.models.smt_resnet import custom_resnet18
...
class BeliefPredictor(nn.Module):
    ...
    def load_pretrained_weights(self):
        ...
        if self.predict_label:
            if self.has_distractor_sound:
                state_dict = torch.load('data/pretrained_ckpts/savi/savi_label_predictor_distractor.pth', weights_only=False)
            else:
                state_dict = torch.load('data/pretrained_ckpts/savi/savi_label_predictor_clean.pth', weights_only=False)
            cleaned_state_dict = {
    ...
    def cnn_forward(self, observations):
        spectrograms = observations[SpectrogramSensor.cls_uuid]
