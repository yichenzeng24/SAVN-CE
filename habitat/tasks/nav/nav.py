...
class TopDownMap(Measure):
    ...
    def __init__(
        ...
        self.line_thickness = int(
            np.round(self._map_resolution * 3 / MAP_THICKNESS_SCALAR)
        )
        self.point_padding = 4 * int(
    ...
    def _draw_goal_position(self, goal):
    ...
    def _draw_distractor_position(self, distractor):
        if self._config.DRAW_DISTRACTOR_POSITIONS and distractor is not None:
            if self._is_on_same_floor(distractor.position[1]):
                try:
                    self._draw_point(
                        distractor.position, maps.MAP_DISTRACTOR_POINT_INDICATOR
                    )
                except AttributeError:
                    pass

    def _draw_goal_aabb(self, goal):
        if self._config.DRAW_GOAL_AABBS:
            try:
                ...
                assert int(
                    ...
                    goal.object_id
                ), f"Object_id doesn't correspond to id in semantic scene objects dictionary for goal: {goal}"
    ...
    def reset_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        if hasattr(episode, "goals"):
            ...
            for goal in episode.goals:
                self._draw_goal_aabb(goal)
                self._draw_goal_position(goal)
            self._draw_shortest_path(episode, agent_position)

        if hasattr(episode, "distractor"):
            self._draw_goal_aabb(episode.distractor)
            self._draw_distractor_position(episode.distractor)
    ...
    def update_map(self, agent_position):
        ...
        if self._top_down_map[a_x, a_y] != maps.MAP_SOURCE_POINT_INDICATOR:
            # silent step, blue; sounding step, yellow
            if self._sim.is_silent:
                color = maps.MAP_SILENT_PATH_COLOR
            else:
                color = maps.MAP_SOUNDING_PATH_COLOR
